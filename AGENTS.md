...
| Server A2A Protocol (HTTP)    | [docs/decisions/server-a2a-http-binding.md](docs/decisions/server-a2a-http-binding.md)               |
...
- **Shared Utilities Convention:** Generic application-wide utilities must reside in `src/utils/` (e.g. `base64.ts`, `coerce.ts`, `delay.ts`, `filename.ts`, `isBinary.ts`, `mime.ts`, `normalizeStringList.ts`, `normalizeWorkspacePath.ts`, `parseBooleanConfig.ts`, `parseStoredStringList.ts`, `ulid.ts`), which is the authoritative root for reusable cross-cutting helpers and exported via package library exports (`./utils`). Complementary operations (such as encode/decode, basename/sanitize, byte/MIME binary detection) should be consolidated into dual-purpose unified functions that cover both operations rather than creating single-function silos. Tool-family-specific utilities shared across multiple tools reside in `src/worker/tools/utils/`. Do not introduce ad-hoc utility directories or duplicate single-function utilities across private `utils/` folders.
...
- Do not call `executeTool` from provider loops without passing the active enabled tool list (`allowedTools`). Runtime allowlist checks, recursion guards, and headless capability checks are enforced in `executeTool` via centralized guards in `src/worker/utils/guards.ts` (`runToolGuards`, `checkAllowedToolGuard`, `checkTaskExecutionGuard`, `checkScheduledTaskBlockedGuard`, `checkHeadlessCapabilityGuard`).
...
- **Declarative Tools:** Definitions are loaded from `.agents/tools/main/**/*.json` and support `bash`, `javascript`, or delegated `tool` executors. Execution is encapsulated in `executeDeclarativeTool` (`src/worker/utils/declarativeToolExecutor.ts`). Preserve built-in-name protection, active runtime allowlists, diagnostics for invalid definitions, and the eight-level delegation limit. Declarative tools (such as `generate_random_number.json` in starter templates) can be provided by content repositories. Update `docs/subsystems/tools.md` when this contract changes.
...
- **Headless CLI Agent Participant (`shadow-claw agent`):**
  ...
  - **Filesystem Storage Backend & Workspace Isolation:** `NodeFsDirectoryHandle` (`src/storage/node-fs-handle.ts`) implements `FileSystemDirectoryHandle` and `FileSystemFileHandle` backed by `node:fs`. Writable streams support strings, `Uint8Array`, `ArrayBuffer`, `Blob`, and `WriteParams`. Both handles implement `isSameEntry()` and throw spec-compliant errors (`TypeMismatchError`, `NotFoundError`, `InvalidModificationError`). Both `_resolveChild` and `parsePath` strictly guard against backwards path traversal (`..` or root escaping) with `SecurityError`. When no `--workspace` is explicitly passed, headless agent bootstrap, init, and WebRTC listener isolate file operations inside an OS sandbox (`<tmpdir>/shadow-claw/workspace`) rather than exposing `.cache` or `process.cwd()`.
  ...
  - **Native AI Task Execution:** Headless execution of built-in AI tasks (`summarize_text`, `rewrite_text`, etc.) is delegated via `handleNativeAiTaskMessage` (`src/cli/commands/native-ai-task-handler.ts`), resolving pending promises via global resolvers.
  - **Host OS Bash Execution & Process Confinement:** In headless mode, `bash` uses `node:child_process` directly via `native-bash-executor.ts`, providing real OS shell access. The child process working directory is strictly bound to the active workspace storage root (`getStorageRootPath()`).
  - **Direct Git Connections in Headless Mode:** In headless Node.js environments, `resolveCorsProxy` returns `undefined` by default to bypass browser CORS proxies for direct HTTPS operations (`gitClone`, `gitPull`, `gitPush`, `gitFetch`), while respecting explicit proxy configurations. `initGitFs` guards checks against `navigator.storage` to avoid runtime TypeErrors in Node.js.
...
- **Stateless MCP Server & Tri-Tier Tool Architecture:** The MCP server engine (`src/cli/commands/mcp.ts`, `src/server/mcp/`) implements the official Stateless Model Context Protocol (2026-07-28) with a tri-tier hierarchy:
  1. `shadowclaw_server_*`: Built-in server management tools (`shadowclaw_server_list_clients`, `shadowclaw_server_send_message`, `shadowclaw_server_status`).
  2. `shadowclaw_local_*`: Host-native CLI agent tools (`read_file`, `write_file`, `bash`, `list_files`, `git_*`) executing directly against host filesystem handles with workspace sandboxing and backwards traversal protection. Enabled by default in `mcp` mode (`--local-tools`), with configurable prefixes (`--tool-prefix local|none`), tool filtering (`--tools`), and profiles (`--tools-profile`). Unprefixed tool calls fallback to host-native local tools when no client is connected.
  3. `shadowclaw_client_*`: Relayed tools from connected browser clients dynamically discovered over the Control Plane (`shadowclaw_client_read_file`, `shadowclaw_client_javascript`), with legacy aliases and client-specific routing (`clientId`).
     Interactive tools like `ask_user` are relayed directly or handled via MRTR (`resultType: "input_required"`). STDIO mode preserves JSON-RPC framing by directing all logs and warnings strictly to `stderr`.
