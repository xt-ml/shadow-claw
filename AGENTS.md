...
- **Remote Tool & Skill Importing (`importArtifacts.ts`):** Supports discovering and importing tools, skills, and companion scripts from RFC v0.2.0 discovery manifests into `.agents/tools/`, `.agents/skills/`, and `.agents/scripts/` in the user's OPFS workspace with SHA-256 verification and auto-enablement.
