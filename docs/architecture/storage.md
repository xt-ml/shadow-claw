...
- **Peer & Room Direct Transfers**: In peer conversations (`peer:...`) or peer rooms (`room:...`), each file row exposes a dedicated "Send to peer" / "Send to room peers" action button (`.files__send-peer`). Selecting send triggers `orchestratorStore.sendFileToPeer()`, streaming file chunks across WebRTC DataChannels and logging the sent attachment in the chat transcript. Peer transfer actions are omitted for directory folders and hidden entirely in non-peer conversations.
