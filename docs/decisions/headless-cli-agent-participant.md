...
4. **CLI Integration.** `shadow-claw agent` subcommands (`init`, `model`, `skills`, `tools`, `tool`, `skill`, `import`, `run`) provide a unified command-line experience for initializing workspaces, managing local ONNX/GGUF models, discovering, inspecting, and importing remote RFC v0.2.0 declarative skills and tools, running skills directly, and composable Unix pipeline integration via stdin piping and `-o`/`-q` flags.
