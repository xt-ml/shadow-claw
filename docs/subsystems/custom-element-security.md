...
- **File Picker Polyfills (`showOpenFilePicker` & `showSaveFilePicker`):** In sandboxed `null`-origin iframes where native File System Access APIs are blocked, `iframe-storage-bridge.js` polyfills `window.showOpenFilePicker` using hidden `<input type="file">` elements and `window.showSaveFilePicker` using synthetic Blob download links, allowing sandboxed custom elements to load and export user files seamlessly.
- **Programmatic Navigation & ServiceWorker Interception (`file-viewer-preview-bridge.js`):** Intercepts link clicks as well as programmatic navigation methods (`location.href`, `location.assign()`, and `location.replace()`) to prevent preview iframes from escaping `srcdoc` context or failing under opaque origins. It also catches and suppresses `SecurityError` rejections from `navigator.serviceWorker` and `window.caches` calls inside the sandbox.
