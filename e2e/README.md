...
- **Peer & Room File Sharing Testing**: When verifying file actions in `<shadow-claw-files>` and `<shadow-claw-file-viewer>`, ensure tests assert that peer transfer options (`.files__send-peer`, `.modal-share-dropdown`) appear strictly when the conversation is a peer (`peer:...`) or room (`room:...`), and remain hidden in non-peer conversations. Verify that clicking peer transfer options correctly calls the underlying channel/store methods, shows typing indicators during transfer, and renders the sent attachment in the local chat stream.
