...
test.describe("File Viewer Modal", () => {
  ...
  test("should display file content in modal when clicking a file from files page", async ({
    ...
    await app.navigateToWithOpenDialog("files");
    ...
    // Some background flows can return the app to chat; force files page before interaction.
    await app.navigateToWithOpenDialog("files");

    // Refresh and click the uploaded file to open the viewer
    const filesComponent = app.filesComponent();
    await filesComponent.locator(".files__refresh-btn").click();
    const fileItem = filesComponent.locator(
      `.files__item-main[aria-label="Open file ${testFileName}"]`,
    );
  ...
  test("should display file modal on files page and persist when navigating to chat", async ({
    ...
    // Refresh and open the uploaded file viewer
    const filesComponent = app.filesComponent();
    await filesComponent.locator(".files__refresh-btn").click();
    const fileItem = filesComponent.locator(
      `.files__item-main[aria-label="Open file ${testFileName}"]`,
  ...
  test("should display different file contents when clicking different files", async ({
    ...
    // Open first file
    const firstItem = filesComponent.locator(
      `.files__item-main[aria-label="Open file ${file1Name}"]`,
    );
    ...
    // Open second file
    const secondItem = filesComponent.locator(
      `.files__item-main[aria-label="Open file ${file2Name}"]`,
    );
  ...
  test("should display file modal on tasks page when opening a file", async ({
    ...
    // Refresh and open the uploaded file
    const filesComponent = app.filesComponent();
    await filesComponent.locator(".files__refresh-btn").click();
    const fileItem = filesComponent.locator(
      `.files__item-main[aria-label="Open file ${testFileName}"]`,
...
async function uploadTestFile(page: Page, fileName: string, content: string) {
  ...
  if (!groupId) {
    throw new Error("No active group ID available for test file upload");
  }

  // Create the file in OPFS
  const created = await page.evaluate(
    async ({ fileName: name, content: text, groupId: activeGroupId }) => {
      console.log("[E2E] Creating file in OPFS:", {
        name,
      ...
      try {
        ...
        const safeId = activeGroupId.replace(/:/g, "-");
    ...
    { fileName, content, groupId },
  );

  if (!created.success) {
    throw new Error(`Failed to create test file: ${(created as any).error}`);
  }

  // Wait for the file to be created
  await page.waitForTimeout(500);

  // Give OPFS a short moment to flush writes before the files page refresh.
  await page.waitForTimeout(300);
}
