...
test.describe("Storage Integration", () => {
  ...
  test("should initialize IndexedDB on page load", async ({ page }) => {
    ...
    expect((dbInitialized as any).dbExists).toBe(true);

    expect((dbInitialized as any).hasStores).toBe(true);
  });
    ...
    const quota = await page.evaluate(async () => {
      ...
      try {
        ...
        return {
          ...
          hasSpace: (estimate.quota as number) > (estimate.usage as number),
        };
      } catch (error) {
        return {
          hasQuotaAPI: true,
  ...
  test("should store and retrieve config via IndexedDB", async ({ page }) => {
    ...
    expect((configAccess as any).hasConfigStore).toBe(true);
  });
});
