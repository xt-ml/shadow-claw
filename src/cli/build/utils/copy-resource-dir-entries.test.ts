...
import { copyResourceDirEntries } from "./copy-resource-dir-entries.js";
...
function neitherEntry(name: string) {
  return {
    name,
    isDirectory: () => false,
    isFile: () => false,
...
describe("copyResourceDirEntries", () => {
  it("copies directories and files from each resource dir", async () => {
    const readdirImpl = jest.fn<any>().mockImplementation(async (dir: any) => {
      if (dir === "resources") {
        return [dirEntry("assets"), fileEntry("README.md")];
      }
      return [];
    });
    const cpImpl = jest.fn<any>().mockResolvedValue(undefined);
  ...
  it("ignores unreadable resource directories", async () => {
    const readdirImpl = jest.fn<any>().mockRejectedValue(new Error("missing"));
    const cpImpl = jest.fn<any>();
  ...
  it("skips non-file, non-directory entries", async () => {
    const readdirImpl = jest
      .fn<any>()
      .mockResolvedValue([neitherEntry("ignored")]);
    const cpImpl = jest.fn<any>();
