...
import { copyWithFallback } from "./copy-with-fallback.js";
...
describe("copyWithFallback", () => {
  ...
  it("accepts a single source string", async () => {
    const statImpl = jest.fn<any>().mockResolvedValue({});
    const cpImpl = jest.fn<any>().mockResolvedValue(undefined);
  ...
  it("returns false when no candidate exists", async () => {
    const statImpl = jest.fn<any>().mockRejectedValue(new Error("missing"));
