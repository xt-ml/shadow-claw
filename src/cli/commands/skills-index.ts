...
async function findFilesRecursively(
  dir: string,
  filterFn?: (name: string, fullPath: string) => boolean,
): Promise<string[]> {
...
export interface GenerateSkillsIndexOptions {
  outDir?: string;
  metadataRoot?: string;
  siteName?: string;
...
export async function generateSkillsIndex(
  ...
  options: GenerateSkillsIndexOptions = {},
  ...
  for (const fullSkillPath of skillFiles) {
    ...
    const associatedToolNames = new Set<string>();
  ...
  const defaultToolchainRoot = getProjectRoot(import.meta.url);
