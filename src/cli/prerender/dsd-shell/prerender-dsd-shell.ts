...
import { DEFAULT_MAIN_GROUP_INDEX_CONTENT } from "../../../storage/defaultIndexContent.mjs";
import { DEFAULT_MAIN_GROUP_MEMORY_CONTENT } from "../../../storage/defaultMemoryContent.mjs";
import { escapeHtml } from "../utils/escape-html.js";
import { sanitizeRenderedHtml } from "../utils/sanitize-rendered-html.js";
import { toPosixPath } from "../utils/to-posix-path.js";
import { normalizePrerenderPagesOption } from "../utils/normalize-prerender-pages-option.js";
...
export interface PrerenderDsdShellOptions {
  indexPath?: string;
  sourcePath?: string;
  ...
  publicDir?: string;
  noSeed?: boolean;
  prerenderPages?: string | number | boolean | null;
...
function buildDefaultPageSource(): PageSource[] {
  return [
    {
      absolutePath: null,
...
export function splitFrontmatterWithGrayMatter(src: string): {
...
export function extractTemplateContent(html: string): string {
...
export async function collectToolSources(
  sourcePath: string,
): Promise<ToolSource[]> {
  try {
    const sourceStats = await stat(sourcePath);
...
export async function renderPageHtml(
  ...
  if (parsed.data && Object.keys(parsed.data).length > 0) {
    const frontmatterMarkup = renderFrontmatterMarkup(
      ...
      createFrontmatterDetailsElement,
      {
        documentInstance: frontmatterDom as unknown as Document,
        serializeNode: (node: any) =>
          frontmatterSerializer.serializeToString(node),
      },
    );
...
function findMatchingTemplateEnd(input: string, startIndex: number): number {
...
function removeExistingShadowClawDsd(innerHtml: string): string {
...
function removeLegacyBootShell(innerHtml: string): string {
...
function buildStaticPagesListMarkup(pageSources: PageSource[]): string {
...
export function injectPageHeaderDsd(
  pagesContent: string,
  pageHeaderTemplateContent: string,
  frontmatterTitle = "",
  pageHeaderCss = "",
): string {
...
function applyNoSeedPagesContent(templateContent: string): string {
  let next = templateContent;
  next = next.replace(
...
function buildPagesDsdHostEmpty(
  pagesTemplateContent: string,
  pagesCss = "",
): string {
...
function wrapShadowClawDialogContentInTemplate(html: string): string {
  return html.replace(
...
export function applySidebarVisibilityToTemplate(
  html: string,
...
export function buildShadowClawDsdTemplate(
  shadowClawTemplateContent: string,
  pagesDsdHost: string,
  shadowClawCss = "",
  sidebarConfig: Record<string, any> = {},
): string {
...
export function injectShadowClawTemplate(
  indexHtml: string,
  dsdTemplate: string,
): string {
...
export function markNoSeedPrerenderHost(indexHtml: string): string {
...
export async function inlineCriticalAssets(
  html: string,
  publicDir: string,
): Promise<string> {
...
export function minifyDsdTemplateHtml(html: string): string {
  ...
  const minifySegment = (content: string) =>
    content.replace(/\s+/g, " ").replace(/>\s+</g, "><").trim();

  function processTemplates(str: string): string {
  ...
  minified = minified.replace(
    ...
    (_match, startTag, innerContent, endTag) => {
...
export async function prerenderDsdShell(
  options: PrerenderDsdShellOptions = {},
): Promise<void> {
  ...
  let manifestPurgeId: string | undefined;
  const purgeTokens: Record<string, any> = {};
  ...
  const fullManifest: Record<string, any> = { pages: fullManifestPages };
  ...
  let dsdPages: PageSource[] = [];
  let embeddedManifestPages: any[] = [];
  ...
  if (filteredPageSources.length === 0) {
  ...
  const embeddedManifest: Record<string, any> = {
...
async function main(): Promise<void> {
  ...
  let prerenderPages: string | undefined;
