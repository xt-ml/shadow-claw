...
export interface SiteConfigPwa {
  name?: string;
  shortName?: string;
...
export interface SiteConfig {
  ...
  assetsDir?: string;
  scripts?: any[];
  allowedCustomElementDomains?: string[];
  customElements?: any;
}
...
function resolveThemeStylesheetHref(stylesheet: string): string {
...
function normalizeManifestIcon(icon: ManifestIcon): ManifestIcon[] {
...
export function patchManifest(
  ...
  if (pwa.screenshots && Array.isArray(pwa.screenshots)) {
    manifest.screenshots = pwa.screenshots.map((screen: any) => {
...
export function patchSitemap(content: string, pagesOrigin?: string): string {
  if (!pagesOrigin) {
    return content;
  }
    ...
    .map((line) => {
      ...
      if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
        try {
          ...
          const basePath = originUrl.pathname.endsWith("/")
...
function getRepoRootDir(configDir: string): string {
...
function getTemplateRootDir(configDir: string): string {
  const resolved = path.resolve(configDir);
...
function getCandidateFilePaths(
...
export async function copyCustomSiteFiles(
  config: SiteConfig,
    ...
    new Set(
      [
        ...rawAssetPaths.flatMap((ap) =>
          getCandidateFilePaths(ap as string, configDir, templateRootDir),
        ),
  ...
  const copiedAssetSources = new Set<string>();
...
export async function copyThemeStylesheet(
  config: SiteConfig,
  distPublicDir: string,
  siteConfigPath?: string,
): Promise<void> {
...
export async function copyLocalScripts(
  config: SiteConfig,
  distPublicDir: string,
  siteConfigPath?: string,
): Promise<void> {
...
export async function copyBrandingAssets(
  config: SiteConfig,
  distPublicDir: string,
  siteConfigPath?: string,
): Promise<void> {
  ...
  if (manifestCandidate) {
    ...
    if (manifestData && Array.isArray(manifestData.icons)) {
      manifestIconSrcs = manifestData.icons
        .map((i: any) => i?.src)
        .filter(Boolean);
    }
  }
  ...
  const copiedDestPaths = new Set<string>();
...
export async function applySiteConfig(
  ...
  const sitemapTxtPath = path.join(distPublicDir, "sitemap.txt");
  const sitemapTxtText = await readText(sitemapTxtPath);
  if (sitemapTxtText) {
    const patched = patchSitemap(sitemapTxtText, pagesOrigin);
    await writeText(sitemapTxtPath, patched);
    console.log("  Patched sitemap.txt");
  }

  const notFoundPath = path.join(distPublicDir, "404.html");
  const notFoundHtml = await readText(notFoundPath);
  if (notFoundHtml) {
    const patched = patch404Html(notFoundHtml, config, basePath);
...
if (isMainModule) {
  main().catch((err) => {
    console.error("site-config/apply.ts failed:", err);
    process.exit(1);
  });
}
