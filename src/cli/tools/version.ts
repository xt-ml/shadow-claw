...
export function parseSemverString(v: string): string | null {
...
export function validateVersionArg(arg?: string): ValidateVersionResult | null {
  if (!arg) return null;
...
export async function syncWellKnownVersions(
  newVersion: string,
  targetRoot: string = rootDir,
): Promise<string[]> {
  const filesToSync = [
