...
import { readStdin, mapTextToToolInput } from "./stdin.js";
...
describe("src/cli/utils/stdin.ts - readStdin", () => {
...
describe("src/cli/utils/stdin.ts - mapTextToToolInput", () => {
  it("maps to `text` when schema has text property", () => {
    const schema = {
  ...
  it("maps to `prompt` when schema has prompt but not text", () => {
    const schema = {
  ...
  it("maps to `content` when schema has content but not text/prompt", () => {
    const schema = {
  ...
  it("falls back to `input` when no priority fields match", () => {
    const schema = {
  ...
  it("falls back to `input` when schema is undefined", () => {
    expect(mapTextToToolInput("hello", undefined)).toEqual({ input: "hello" });
  });

  it("falls back to `input` when schema has no properties field", () => {
    expect(mapTextToToolInput("hello", { type: "object" })).toEqual({
      input: "hello",
    });
  });

  it("prefers `text` over `prompt` when both are present", () => {
    const schema = {
  ...
  it("prefers `prompt` over `content` when both are present", () => {
    const schema = {
