...
import type { ButtonSpec } from "../../../../ui/a2ui/types.js";
...
export function renderButton(
  ...
  btn.addEventListener("click", () => {
    ...
    // Spec §actions: use event.name; fall back to deprecated id field.
    const actionId =
      spec.action.event?.name ?? spec.action.id ?? (spec.action as any).name;
    if (actionId) {
