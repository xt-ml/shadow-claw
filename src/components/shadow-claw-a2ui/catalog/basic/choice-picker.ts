...
import type { ChoicePickerSpec } from "../../../../ui/a2ui/types.js";
...
export function renderChoicePicker(
  ...
  if (typeof spec.value === "object" && "path" in spec.value) {
    ...
    boundKey = (spec.value as { path: string }).path.replace(/^\//, "");
    ...
    boundKey = (spec.value as { $dataModel: string }).$dataModel.replace(
      /^\//,
      "",
    );
    const current = surface.dataModel[boundKey];
