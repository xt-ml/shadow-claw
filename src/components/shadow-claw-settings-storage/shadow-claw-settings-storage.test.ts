...
jest.unstable_mockModule("../../storage/requestPersistentStorage.js", () => ({
...
const { ShadowClawSettingsStorage } =
  await import("./shadow-claw-settings-storage.js");

describe("shadow-claw-settings-storage", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it("registers custom element", () => {
    expect(customElements.get("shadow-claw-settings-storage")).toBe(
      ShadowClawSettingsStorage,
    );
  });

  it("renders storage usage", async () => {
    const el = new ShadowClawSettingsStorage();
