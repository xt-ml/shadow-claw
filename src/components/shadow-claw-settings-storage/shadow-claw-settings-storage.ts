...
const elementName = "shadow-claw-settings-storage";

/**
 * Settings sub-component for Storage configuration:
 * usage display, persistent storage, directory selection.
 */
export class ShadowClawSettingsStorage extends ShadowClawElement {
  static componentPath = `components/${elementName}`;
  static styles = `${ShadowClawSettingsStorage.componentPath}/${elementName}.css`;
  static template = `${ShadowClawSettingsStorage.componentPath}/${elementName}.html`;
...
customElements.define(elementName, ShadowClawSettingsStorage);
