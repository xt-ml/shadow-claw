...
const { readGroupFileBytes } = await import("./readGroupFileBytes.js");
...
describe("readGroupFileBytes", () => {
  ...
  it("reads bytes via OPFS sync access handle when available", async () => {
    ...
    const content = await readGroupFileBytes({} as any, "g1", "nested/a.pdf");
  ...
  it("falls back to File API arrayBuffer when sync access is unavailable", async () => {
    ...
    (mockGetGroupDir as any).mockResolvedValue(groupDir);
    (mockParsePath as any).mockReturnValue({ dirs: [], filename: "b.bin" });

    const content = await readGroupFileBytes({} as any, "g2", "b.bin");
