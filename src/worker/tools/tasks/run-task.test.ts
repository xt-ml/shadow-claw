...
const { executeRunTask } = await import("./run-task.js");
...
describe("executeRunTask", () => {
  ...
  it("returns error if task not found", async () => {
    ...
    const result = await executeRunTask({} as any, { id: "task-1" }, "group-1");
  ...
  it("runs task successfully", async () => {
    ...
    expect(result).toBe("Task task-1 triggered successfully.");
  });
});
